...
/*   Updated: 2023/07/27 15:46:16 by lpicoli-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (str[x->i] == '$' && (ft_is_valid_character(str[x->i + 1]) || str[x->i + 1] == '?'))
	{
		ft_add_new_elem(elem_head, ft_new_elem(str + x->i,
											   ft_count_char_env(x->ms, str + x->i), ENV, status));
		x->i = x->i + ft_count_char_env(x->ms, str + x->i) - 1;
	}
