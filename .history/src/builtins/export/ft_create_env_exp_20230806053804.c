...
/*   Updated: 2023/08/06 05:38:04 by ialves-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_create_exp(t_ms *ms, char **env)
{
	...
	*env_head = list;
	ft_creat_exp_while(ms, exp, &env);
	ms->export_list = *env_head;
	free (env_head);
	free (exp);
}

void	ft_creat_exp_while(t_ms *ms, t_export *exp, t_lst **env)
{
	t_lst	*list_env;
	...
	while (exp->lower_case <= 'z')
	{
		...
		while (env[ms->i])
		{
			if (list_env[ms->i][0] == exp->upper_case || env[ms->i][0] == exp->lower_case
